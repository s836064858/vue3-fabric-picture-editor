...
- **AI 消除**：涂抹/框选/圈选指定区域，生成自然填充结果。
- **一键抠图（去除背景）**：对主对象进行抠图，输出透明背景 PNG。
- **图层规则**：若存在多图层，仅对「最底层可见图层」生效；生成完成后会隐藏原图层，并新增一个结果图层。
- **API Key 获取**：在工具栏 AI 面板输入 API Key（支持粘贴），可从火山方舟控制台获取：
...
- **正常导出**：点击右上角「导出图片」会将画布导出为 PNG。
- **AI 结果导出**：AI 返回可能包含外链图片地址，项目会自动下载并转为 DataURL 再加入画布，以避免跨域污染导致导出失败。
- **异常提示**：若浏览器拦截外链图片下载（跨域限制等），导出会失败并给出提示。
